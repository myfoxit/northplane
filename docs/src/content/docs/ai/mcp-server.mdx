...
| `operate` | Read + operate / Lesen + Bedienen | read set + `alerts:ack,objects:write,maintenance:write` |
| `configure` | Read + configure / Lesen + Konfigurieren | read set + `config:write,oncall:write` |

:::caution[The "operate" preset does not cover downtimes, silences or rechecks]
`maintenance:write` is not a permission any tool or REST route checks; downtimes need `downtimes:write`, silences `silences:write` and `run_check_now` needs `checks:run`. With the `operate` preset the only permitted mutating tool is `acknowledge_alert`. Add `downtimes:write`, `silences:write` and `checks:run` explicitly (or mint the token with the `ai-agent` role) when the agent should schedule downtimes, silence alerts or trigger rechecks. `objects:write` in the preset is not used by any tool either.
:::
