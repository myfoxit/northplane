...
Hovering a row reveals three actions: **Check now** (Jetzt prüfen, icon; `POST /api/v1/objects/{id}/check-now`, `checks:run`), **Edit** (Bearbeiten; opens the dialog below in edit mode) and **Delete** (Löschen; `DELETE /api/v1/objects/{id}`, `objects:write`). Delete is a two-click inline confirmation — the button turns into "Really delete?" (Wirklich löschen?) — and for hosts adds the warning "Also deletes all services on this host?" (Löscht auch alle Services des Hosts?): deleting a host cascades to its services.
