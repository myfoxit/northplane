...
| `docker` | — (runs in parallel) | buildx (QEMU is set up, but the Dockerfile cross-compiles so only the distroless runtime stage is per-platform), `docker/login-action` to `ghcr.io` with `GITHUB_TOKEN`, `docker/metadata-action` tags `type=semver,pattern={{version}}`, `type=semver,pattern={{major}}.{{minor}}`, `type=raw,value=latest`; `docker/build-push-action` with `platforms: linux/amd64,linux/arm64`, `build-args: VERSION=${{ github.ref_name }}`, GHA layer cache |
