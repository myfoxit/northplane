...
| Web UI | port 8006, IP-allowlisted in `cluster.fw` (see [access patterns](/docs/deployment/proxmox-vm/#access-patterns)) |
...
- The origin answers directly as well: `curl --resolve doktrace.com:443:51.83.96.40 https://doktrace.com/healthz` works — the WAF is not an enforcement boundary.
...
| use the Proxmox web UI | `ssh -N -L 8006:127.0.0.1:8006 root@51.83.96.40` then `https://localhost:8006` (realm `pam`) — the firewall allowlists 8006 per source IP and silently drops everything else, so the tunnel is the durable way |
