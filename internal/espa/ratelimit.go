...
// internal/traps carries the same copy). Defaults match the ingress
// adapter so pager rate limits behave identically.
type bucket struct {
	mu     sync.Mutex
	tokens float64
	last   time.Time
...
// allowRate reports whether a call from sourceID may pass, refilling the
