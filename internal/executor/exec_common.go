...
//   - hard timeout: context cancel + kill of the whole process group
//     (setProcAttr is platform-specific; see exec_unix.go / exec_windows.go)
...
func (ex *Executor) runExec(ctx context.Context, job scheduler.Job, e *catalog.Entry) {
	...
	env := []string{"PATH=" + defaultPluginPath, "LC_ALL=C"}
